# Backlog
- active: 8
- areas: { durability: 2, recovery: 2, idempotency: 2, queues: 0, messaging: 1 }
- top-score: 384
- threshold: 20
...
- **queues: FINDING** — sqlite-two-runner-dequeue-exactly-once (#541), raw score
  512. Was parked "off-directive (brief pins Postgres)"; the runtime is
  musl+gcompat where the Postgres path is DOA, so the SQLite backend became the
  ONLY viable surface and this candidate was promoted to promise
  `queue-exactly-once`. Reproduced RED officially: #541's two-runner
  double-dequeue survives #564's isolation fix. See map.md promoted findings.
- queues: the earlier rate-limiter over-admission (former `queue-limits-global`)
  was reproduced LOCALLY (Postgres, 8 executors admit 4 in a limit-2 window) but
  is NOT officially runnable — the musl runtime blocks psycopg/pgserver. Retired
  in favor of queue-exactly-once; recorded as a local-only observation in SUMMARY.
- durability: 1 promoted (durable-workflow-completion baseline + crash-recover),
  both GREEN officially on SQLite/musl.
